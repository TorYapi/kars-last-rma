...
-- Drop the unique constraint on session_id (this will also drop the associated index)
ALTER TABLE user_sessions DROP CONSTRAINT IF EXISTS user_sessions_session_id_key;

-- Create a regular (non-unique) index for performance if it doesn't exist
CREATE INDEX IF NOT EXISTS idx_user_sessions_session_id_regular ON user_sessions (session_id);
